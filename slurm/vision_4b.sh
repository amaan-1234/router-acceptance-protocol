#!/bin/bash
#SBATCH --job-name=vision_4b
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=40G
#SBATCH --time=04:00:00
#SBATCH --output=logs/vision_4b_%j.log
...
MODEL_PATH=$HOME/.cache/huggingface/hub/models--Qwen--Qwen3.5-4B/snapshots/851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a
PORT=8100
...
$VLLM serve $MODEL_PATH \
    ...
    --limit-mm-per-prompt '{"image":1}' > logs/vllm_4b_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_4b_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_4b_${SLURM_JOB_ID}.log; exit 1; fi
done
