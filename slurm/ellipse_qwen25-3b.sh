#!/bin/bash
#SBATCH --job-name=ell_q25
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --time=08:00:00
#SBATCH --output=logs/ell_q25_%j.log
...
echo "Starting vLLM for qwen25-3b on port $PORT..."
$VLLM serve $MODEL_PATH \
    ...
    --dtype bfloat16 \
    --enforce-eager \
    --gpu-memory-utilization 0.92 --max-num-seqs 16  > logs/vllm_qwen25-3b_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_qwen25-3b_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_qwen25-3b_${SLURM_JOB_ID}.log; exit 1; fi
done
