#!/bin/bash
#SBATCH --job-name=ell_phi35
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --time=08:00:00
#SBATCH --output=logs/ell_phi35_%j.log
...
MODEL_DIR=/scratch/akalemul/hf_cache/hub/models--microsoft--Phi-3.5-mini-instruct
...
echo "Starting vLLM for phi35-mini on port $PORT..."
$VLLM serve $MODEL_PATH \
    --served-model-name phi35-mini \
    --port $PORT \
    --max-model-len 8192 \
    --dtype bfloat16 \
    --enforce-eager \
    --gpu-memory-utilization 0.92 --max-num-seqs 16 --trust-remote-code > logs/vllm_phi35-mini_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_phi35-mini_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_phi35-mini_${SLURM_JOB_ID}.log; exit 1; fi
done
