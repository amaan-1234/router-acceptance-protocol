#!/bin/bash
#SBATCH --job-name=vision_2b
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=40G
#SBATCH --time=04:00:00
#SBATCH --output=logs/vision_2b_%j.log
...
MODEL_PATH=$HOME/.cache/huggingface/hub/models--Qwen--Qwen3.5-2B/snapshots/15852e8c16360a2fea060d615a32b45270f8a8fc/
PORT=8102
...
echo "Starting vLLM ($($VLLM --version 2>&1)) for Qwen3.5-2B on port $PORT (FlashInfer sampler OFF)..."
$VLLM serve $MODEL_PATH \
    ...
    --limit-mm-per-prompt '{"image":1}' > logs/vllm_2b_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_2b_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_2b_${SLURM_JOB_ID}.log; exit 1; fi
done
