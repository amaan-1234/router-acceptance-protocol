#!/bin/bash
#SBATCH --job-name=vision_27b
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:2
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=08:00:00
#SBATCH --output=logs/vision_27b_%j.log
...
MODEL_PATH=/scratch/akalemul/hf_cache/hub/models--Qwen--Qwen3.5-27B/snapshots/fc05daec18b0a78c049392ed2e771dde82bdf654
PORT=8103
...
echo "Starting vLLM ($($VLLM --version 2>&1)) for Qwen3.5-27B on port $PORT (FlashInfer sampler OFF)..."
$VLLM serve $MODEL_PATH \
    ...
    --dtype bfloat16 \
    --enforce-eager \
    --gpu-memory-utilization 0.90 --tensor-parallel-size 2 --max-num-seqs 16 \
    --limit-mm-per-prompt '{"image":1}' > logs/vllm_27b_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_27b_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_27b_${SLURM_JOB_ID}.log; exit 1; fi
done
