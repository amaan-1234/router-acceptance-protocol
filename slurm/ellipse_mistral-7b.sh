#!/bin/bash
#SBATCH --job-name=ell_mistral
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --time=08:00:00
#SBATCH --output=logs/ell_mistral_%j.log
...
MODEL_DIR=/scratch/akalemul/hf_cache/hub/models--mistralai--Mistral-7B-Instruct-v0.3
...
echo "Starting vLLM for mistral-7b on port $PORT..."
$VLLM serve $MODEL_PATH \
    ...
    --dtype bfloat16 \
    --enforce-eager \
    --gpu-memory-utilization 0.92 --max-num-seqs 16  > logs/vllm_mistral-7b_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_mistral-7b_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_mistral-7b_${SLURM_JOB_ID}.log; exit 1; fi
done
