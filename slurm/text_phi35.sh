#!/bin/bash
#SBATCH --job-name=text_phi35
#SBATCH --partition=public
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=40G
#SBATCH --time=04:00:00
#SBATCH --output=logs/text_phi35_%j.log
...
MODEL_PATH=/scratch/akalemul/hf_cache/hub/models--microsoft--Phi-3.5-mini-instruct/snapshots/2fe192450127e6a83f7441aef6e3ca586c338b77
PORT=8113
...
$VLLM serve $MODEL_PATH \
    --served-model-name phi35-mini --trust-remote-code \
    --port $PORT \
    --max-model-len 4096 \
    --dtype bfloat16 \
    --enforce-eager \
    --gpu-memory-utilization 0.85 --max-num-seqs 32 \
    --limit-mm-per-prompt '{"image":1}' > logs/vllm_phi_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
...
for i in $(seq 1 90); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then echo "vLLM died — see logs/vllm_phi_${SLURM_JOB_ID}.log"; tail -40 logs/vllm_phi_${SLURM_JOB_ID}.log; exit 1; fi
done
