"""Does model grading-disagreement track instructor disagreement? (Mohler, Phase 2)

The grading analogue of run/report.py + run/eval_selective.py, on real per-rater data.
Reads outputs/mohler_graders.jsonl (from run_mohler_graders) and reports:

  E1 signal validity : Pearson/Spearman of model disagreement (std or gap across the
                       K grader personas) vs instructor disagreement (|rater1-rater2|).
                       This is the direct Phase-1-style question on grading data.

  E2 selective grading: rank items by model disagreement, "escalate" the top fraction
                       to a reference grade (here the instructor average = adjudicated
                       truth), rest graded by the model panel mean rounded to scale.
                       Accuracy = within-0.5 of the adjudicated grade. Disagreement-
                       ranked escalation vs random escalation at matched budget.

  E3 calibration check: mean instructor gap within each model-disagreement quartile
                       (does higher model spread really mean harder-for-humans items?).

No API calls — pure post-processing. Run after the grader ensemble finishes.

Usage:
    python -m run.eval_mohler
    python -m run.eval_mohler --tolerance 0.5 --signal model_std
"""
...
try:
    ...
    DEFAULT_IN = config.OUTPUTS_DIR / "mohler_graders.jsonl"
    DEFAULT_OUT = config.OUTPUTS_DIR / "mohler_eval.json"
except Exception:
    DEFAULT_IN = Path("outputs/mohler_graders.jsonl")
    DEFAULT_OUT = Path("outputs/mohler_eval.json")


def _load(path):
    rows = [json.loads(l) for l in Path(path).read_text().splitlines() if l.strip()]
    if not rows:
        raise SystemExit(f"no records in {path}; run run_mohler_graders first")
    return rows
...
def e2_selective(model_sig, model_grade, adjudicated, tol, n_steps=21, n_rand=200, seed=0):
    ...
    order = np.argsort(-model_sig)                 # most model-disagreement first
    model_correct = np.abs(model_grade - adjudicated) <= tol  # model right within tol
    rows = []
    for b in np.linspace(0, 1, n_steps):
        k = int(round(b * n))
        esc = np.zeros(n, bool); esc[order[:k]] = True
        # escalated -> adjudicated (correct by definition); else model grade
...
def main():
    ...
    adjudicated = np.array([r["score_avg"] for r in rows], dtype=float)
    ...
    bar = "=" * 70
    print(f"\n{bar}\nMOHLER GRADING DISAGREEMENT  (N={len(rows)}, signal={args.signal})\n{bar}")
