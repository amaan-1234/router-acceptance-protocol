...
escalating contested items to an expensive frontier model (a large, high-capacity model serving as the escalation tier) or human. We argue this
...
(AURC) and selective accuracy (the model's accuracy restricted to the fraction of items it chooses to answer) under a budget, not only the
...
cascades --- multi-tier systems that route each input to the cheapest model capable of handling it \citep{viola2001rapid} --- and FrugalGPT \citep{chen2023frugalgpt} invoke
...
pre-router's false-easy rate --- the fraction of hard items incorrectly passed to the cheap tier as easy --- falls from ${\approx}37\%$ at $N{=}500$ to $1.2\%$ at
...
Qwen3.5-4B$^{\ast}$    & $0.377$ & $0.230$ & $0.787$ & $0.686$ \\
Qwen3.5-9B             & $0.304$ & $0.306$ & $0.618$ & $0.584$ \\
Qwen3.5-27B            & $0.271$ & $0.438$ & $0.520$ & $0.476$ \\
Qwen3.5-122B (MoE)$^{\dagger}$ & $0.268$ & $0.387$ & $0.596$ & $0.495$ \\
\midrule
Gemma4-E2B$^{\S}$   & $0.150$ & $0.375$ & $0.644$ & $0.600$ \\
Gemma4-31B$^{\S}$   & $0.383$ & $0.158$ & $0.822$ & $0.813$ \\
Qwen3.6-27B$^{\S}$  & $0.352$ & $\mathbf{0.146}$ & $0.853$ & $0.810$ \\
\midrule
Frontier (Sonnet 4.6)  & $\mathbf{0.471}$ & $0.178$ & --- & --- \\
\bottomrule
\end{tabular}%
}
...
decision of its own.
$^{\S}$Cross-family comparison model; not placed on the Qwen3.5 capacity curve.}
\label{tab:sweep}
\end{table}
...
As a cross-family check, Gemma4-31B ($r{=}0.545$, AURC$=0.174$) and
Qwen3.6-27B ($r{=}0.486$, AURC$=0.152$) both land near the top of the
vision correlation range (Table~\ref{tab:vision_sweep}), consistent with
the general trend that more capable models track human disagreement better
and confirming that the A2 capacity trend is not an artefact of the Qwen3.5
backbone.
...
to future work.
Cross-family models reinforce this picture: Gemma4-31B and Qwen3.6-27B show
moderate pairwise $\phi$ with the Qwen3.5 vision sweep models
($\phi{\in}[0.30, 0.45]$) --- well below the ChaosNLI small-dense block
($\phi{=}0.78$--$0.83$) but above the nearly-independent $0.8$B--$2$B
Qwen3.5 pair ($\phi{=}0.08$), consistent with a mixed-correlation regime
in the vision setting rather than the tight correlated-error block seen in
language.
...
\midrule
Gemma4-31B$^{\S}$   & $0.545$ & $0.174$ & $0.776$ & $0.717$ \\
Qwen3.6-27B$^{\S}$  & $0.486$ & $0.152$ & $0.769$ & $0.771$ \\
\bottomrule
\end{tabular}
